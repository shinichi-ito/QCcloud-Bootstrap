...
export class KoukaListAllComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.data=this.insideService.koukaList
  }
  setMoto(item){
    let claimItem;
...
let taisakuData;
    for(let key in this.taisakuList) {
      if (item.taisakukey == this.taisakuList[key].key) {
//console.log(this.taisakuList[key])
        taisakuData=this.taisakuList[key];
      }
    }
this.taisakuData=taisakuData;
